...
def calculate_point(data_file, groud_trust):
    ...
    metric_of_algorithm = [[result_dta_bayes.anomaly_score, "Bayes Algorithm"],
                           [result_dta_relativeE.anomaly_score, "Relative Entropy Algorithm"],
                           [breakpoint_candidates, "EDGE Algorithm"],
                           [result_dta_skyline.anomaly_score, "Skyline Algorithm"],
                           [result_dta_numenta.anomaly_score, "Numenta Algorithm"],
                           [result_dta_contextOSE.anomaly_score, "ContextOSE Algorithm"]
                           ]
...
def anomaly_detection(result_dta, raw_dta, filed_name, alpha, groud_trust, data_file='dta_tsing', debug_mode=0):
    ...
    print("Y calculatime time: {}".format(time.time()-start_time_y))
    # Find normal point
    ...
    print("Z calculation time: {}".format(time.time() - start_time_z))
...
def online_anomaly_detection(result_dta, raw_dta, alpha):
    ...
    X = list(map(lambda x: [x, result_dta.values[x][1]], np.arange(len(result_dta.values))))
    # dt=DistanceMetric.get_metric('pyfunc',func=mydist)
    tree = nb.KDTree(X, leaf_size=100)
    potential_anomaly = []
    # Calculate Y
    for anomaly_point in anomaly_index:
        if anomaly_point - 1 not in potential_anomaly:
